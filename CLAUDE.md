...
- Cloudflare Workers AI FLUX.1 [schnell] for graphics (free, commercial-clean); transparency via @imgly/background-removal-node (server-side cutout)
...
- Image model is Cloudflare Workers AI FLUX.1 [schnell] (free), called server-side from /api/generate. FLUX has no transparent output, so the design-director requests a flat light-grey background and the route cuts it out with @imgly/background-removal-node → transparent PNG (data URL). OpenAI/gpt-image-1 removed.
